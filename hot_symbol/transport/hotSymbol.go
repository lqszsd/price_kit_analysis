...
func NewHttpHandler(endpoint end_point.EndPoint, log *zap.Logger) http.Handler {
	...
	m := http.NewServeMux()
	m.Handle("/api/hot/list", httptransport.NewServer(
		endpoint.HotListEndPoint,
		decodeHTTPHotLostRequest,    //解析请求值
...
func errorEncoder(ctx context.Context, err error, w http.ResponseWriter) {
	...
	json.NewEncoder(w).Encode(errorWrapper{Error: err.Error(),Code:"200"})
}
...
func decodeHTTPHotLostRequest(ctx context.Context, r *http.Request) (interface{}, error) {
	return nil, nil
}
